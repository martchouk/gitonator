...
### Summary
The plan covers the core behavior change, regression tests, and README update. The blocker is the suggested brace-stripping implementation: as written, it would also accept malformed forms like `$VAR}` and `${VAR`, which changes existing error behavior and could hide configuration mistakes. Tighten the parsing so only exact `${VAR}` is normalized.

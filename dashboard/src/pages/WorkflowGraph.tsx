            ...
            return (
              <React.Fragment key={status}>
                ...
                >
                  ...
                  <span style={pathNodeRole(node?.role)}>{prettyRole(node?.role)}</span>
                </button>
              </React.Fragment>
            );
          })}
...
const segmentButton = (active: boolean): React.CSSProperties => ({
  ...
  borderRadius: 'var(--radius-sm)',
  padding: '5px 12px',
  color: active ? 'var(--workflow-selector-active-fg)' : 'var(--md-sys-color-on-surface-variant)',
  background: active ? 'var(--workflow-selector-active-bg)' : 'transparent',
  cursor: 'pointer',
  fontFamily: 'var(--font-sans)',
  fontSize: '0.75rem',
  fontWeight: 600,
...
const pathNodeRole = (role?: string): React.CSSProperties => ({
  color: roleColor(role),
  fontSize: '0.75rem',
  textTransform: 'none',
  letterSpacing: 0,
